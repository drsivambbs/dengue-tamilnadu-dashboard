...
function Timeline({ selected }: { selected: string | null }) {
  ...
  const data = useMemo(() => {
    const rows: { key: string; label: string; value: number; avg: number | null }[] = []
    for (let y = from.y; y <= to.y; y++) {
      ...
      for (let m = mStart; m <= mEnd; m++) {
        rows.push({ key: `${y}-${m}`, label: `${MONTHS[m]} '${String(y).slice(2)}`, value: series[m] ?? 0, avg: null })
      }
    }
        ...
        {data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 12, right: 20, bottom: 56, left: 8 }} barCategoryGap="18%">
              ...
              <Bar dataKey="value" name={metricLabel} fill="#1f5fa6" stroke="#000000" strokeWidth={0.35} radius={[2, 2, 0, 0]} isAnimationActive={false} />
              <Line dataKey="avg" name="3-mo average" type="monotone" stroke="#c0392b" strokeWidth={2} dot={false} isAnimationActive={false} />
            </ComposedChart>
          </ResponsiveContainer>
        )}
      </div>

      <div className="flex items-center gap-4 border-t border-line px-5 py-2 text-[0.8rem] text-ink-soft">
